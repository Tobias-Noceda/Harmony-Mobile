...
class ColorPickerController {
  ...
  private val selectedColor: State<Color> = _selectedColor

  /** State of [Color], which represents the currently selected color value without alpha and brightness. */
  private var pureSelectedColor: MutableState<Color> = mutableStateOf(Color.Transparent)

  /** Alpha value to be applied with the selected color. */
  private var alpha: MutableState<Float> = mutableFloatStateOf(1.0f)

  /** Brightness value to be applied with the selected color. */
  private var brightness: MutableState<Float> = mutableFloatStateOf(1.0f)
  ...
  private var isAttachedAlphaSlider: Boolean = false

  /** Indicates if the brightness slider has been attached. */
  private var isAttachedBrightnessSlider: Boolean = false
  ...
  fun setPaletteImageBitmap(imageBitmap: ImageBitmap) {
    ...
    if (isHsvColorPalette) {
      selectByColor(_selectedColor.value)
    } else {
      selectCenter()
    }
    reviseTick.intValue++
   ...
   fun selectByCoordinate(x: Float, y: Float, fromUser: Boolean) {
    ...
    if (extractedColor != Color.Transparent) {
      ...
      if (fromUser && debounceDuration != 0L) {
        notifyColorChangedWithDebounce()
      } else {
   ...
   */
  private fun selectByColor(color: Color) {
    ...
    if (palette != null) {
      ...
      if (pickerRadius > 0) {
        ...
        selectByCoordinate(x.toFloat(), y.toFloat(), false)
  ...
  private fun selectCenter() {
    ...
    selectByCoordinate(size.width * 0.5f, size.height * 0.5f, false)
  }
  ...
  private fun notifyColorChanged(fromUser: Boolean) {
    ...
    colorChangedTick.value = ColorEnvelope(color, color.toArgb().toHexString(), fromUser)
  }

  /** Notify color changes to the color picker and other subcomponents with debounce duration. */
  private fun notifyColorChangedWithDebounce() {
    val runnable = { notifyColorChanged(true) }
